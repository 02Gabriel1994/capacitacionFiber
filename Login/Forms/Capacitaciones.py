...
from tkinter import ttk, Canvas, Scrollbar, Menu, messagebox
...
class RCapacitacion:
    ...
    def __init__(self, root, principal):
        ...
        #Se crea el canvas.
        self.canvas = Canvas(self.ventana, bg="#ffcc00")
        self.canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.scrollbar = Scrollbar(self.ventana, command=self.canvas.yview)
        self.scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.canvas.config(yscrollcommand=self.scrollbar.set)
        #--------------
        #Segundo frame
        #-----------
        self.content_frame = tk.Frame(self.canvas, bg="#ffcc00")
        self.content_frame.pack(side='top', fill=tk.BOTH, expand=True, padx=100)
        self.canvas.create_window((0, 0), window=self.content_frame, anchor="nw")
                 
        #Etiqueta y cuadro para solicitante
        solicitante = tk.StringVar()
        etiquetaNombre = tk.Label(self.content_frame, text="Solicitante: ", font=('Arial', 10, 'bold'),bg='#ffcc00')
        etiquetaNombre.grid(row=0, column=0, pady=20)
        textoSolicitante = ttk.Combobox(self.content_frame, font=("Arial", 10), textvariable=solicitante, width=35, state="readonly" )
        ...
        label_from = tk.Label(self.content_frame, text="Fecha solicitada:" , font=('Arial', 10, 'bold'),bg='#ffcc00')
        label_from.grid(row=0, column=2, pady=20)
        cal_from = DateEntry(self.content_frame, width=12,  background='black', foreground='#ffcc00', borderwidth=2, state="readonly")
        cal_from.grid(row=0, column=3)
        cal_from.place_forget() 
        
         #Etiqueta y cuadro de fecha para hasta
        label_from = tk.Label(self.content_frame, text="Fecha planeada:" , font=('Arial', 10, 'bold'),bg='#ffcc00')
        label_from.grid(row=0, column=4, pady=20)
        cal_from = DateEntry(self.content_frame, width=12,  background='black', foreground='#ffcc00', borderwidth=2, state="readonly")
        ...
        label_from = tk.Label(self.content_frame, text="Fecha ejecutada:" , font=('Arial', 10, 'bold'),bg='#ffcc00')
        label_from.grid(row=1, column=0, pady=20)
        cal_from = DateEntry(self.content_frame, width=12,  background='black', foreground='#ffcc00', borderwidth=2, state="readonly")
        cal_from.grid(row=1, column=1)
        cal_from.place_forget() 
        #Etiqueta y texto para Tema
        etiquetaTema = tk.Label(self.content_frame, text="Tema:", font=('Arial', 10, 'bold'), bg='#ffcc00')
        etiquetaTema.grid(row=1, column=2, pady=20)
        textoTema = tk.Text(self.content_frame, font=("Arial", 10), width=60, height=2)
        textoTema.grid(row=1, column=3, padx=(0)) 
     
        #--------------
        #cuarto frame
        #-----------   
          #Etiqueta y texto para tipo objetivo
        etiquetaObjetivo = tk.Label(self.content_frame, text="Objetivo:", font=('Arial', 10, 'bold'), bg='#ffcc00')
        etiquetaObjetivo.grid(row=1, column=4, pady=20)
        textoObjetivo = tk.Text(self.content_frame, font=("Arial", 10), width=60, height=2)
        textoObjetivo.grid(row=1, column=5, padx=(0)) 
         #Etiqueta y texto para proveedor
        # etiquetaEmpleado = tk.Label(self.content_frame, text="Proveedor:", font=('Arial', 10, 'bold'), bg='#ffcc00')
        # etiquetaEmpleado.grid(row=3, column=0, pady=20)
        # textoEmpleado = tk.Entry(self.content_frame, font=("Arial", 10), width=35)
      ...
      #Etiqueta y texto para ciudad
        # etiquetaEmpleado = tk.Label(self.content_frame, text="Ciudad:", font=('Arial', 10, 'bold'), bg='#ffcc00')
        ...
        # etiquetaCategoria = tk.Label(self.content_frame, text="Categoria : ", font=('Arial', 10, 'bold'),bg='#ffcc00')
        # etiquetaCategoria.grid(row=4, column=0, pady=20 )
        # textoSolicitante = ttk.Combobox(self.content_frame, font=("Arial", 10), textvariable=categoria, width=35, state="readonly" )
        ...
        # etiquetaCategoria = tk.Label(self.content_frame, text="Tipo capacitacion : ", font=('Arial', 10, 'bold'),bg='#ffcc00')
        # etiquetaCategoria.grid(row=4, column=2, pady=20)
        # textoSolicitante = ttk.Combobox(self.content_frame, font=("Arial", 10), textvariable=capacitacion, width=35, state="readonly" )
        ...
        # etiquetaCategoria = tk.Label(self.content_frame, text="Prioridad : ", font=('Arial', 10, 'bold'),bg='#ffcc00')
        # etiquetaCategoria.grid(row=5, column=0, pady=20)
        # textoSolicitante = ttk.Combobox(self.content_frame, font=("Arial", 10), textvariable=prioridad, width=35, state="readonly" )
        # elementos = ["A", "B","C"]
        # textoSolicitante['values'] = elementos
        # textoSolicitante.grid(row=5, column=1, padx=(1,20))
        # #Etiqueta y cuadro para Evaluacion
        # evaluacion = tk.StringVar()
        # etiquetaCategoria = tk.Label(self.content_frame, text="Evaluacion? : ", font=('Arial', 10, 'bold'),bg='#ffcc00')
        # etiquetaCategoria.grid(row=5, column=2, pady=20)
        # textoSolicitante = ttk.Combobox(self.content_frame, font=("Arial", 10), textvariable=evaluacion, width=35, state="readonly" )
        # elementos = ["SI", "NO"]
        # textoSolicitante['values'] = elementos
        # textoSolicitante.grid(row=5, column=3, pady=20)
        
        #    # otro frame para hora y mn 
        # frameForm5_1 =tk.Frame(self.content_frame, height=70, bd=20,  bg='#ffcc00')#crea un marco frameFormTop dentro de otro marco llamado frameForm. El parámetro height=50 establece la altura del marco en 50 píxeles, bd=0 establece el ancho del borde en cero, relief=tk.SOLID define el estilo del borde como sólido y bg='black' establece el color de fondo del marco en negro.
